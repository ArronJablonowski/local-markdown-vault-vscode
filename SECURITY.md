...
Pasted and dropped attachments pass the same signature and dimension checks before a vault file is created. One operation accepts 1–32 raster files, with a 20 MiB decoded-size limit per file, a 40 MiB aggregate limit, stable input ordering, generated collision-avoiding names, and rollback if the batch cannot complete atomically.
