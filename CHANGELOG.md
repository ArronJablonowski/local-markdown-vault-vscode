...
- Localize the Command Palette category and add AST-backed language gates covering every manifest command, setting, view, and trust description plus argument completeness and placeholder parity for all extension-host and webview US English messages.
...
- Localize the Marketplace identity, custom-editor name, activity-bar container, and Settings section through VS Code's localization API, with catalog-symmetry and manifest-placeholder regression gates. Mermaid and draw.io glyph controls now expose descriptive accessible names, including a fit/native toggle whose name follows its state. HTTPS image opt-in has browser coverage proving that a permitted HTTPS request cannot redirect into an HTTP tracking request.
...
- Declare every webview document as US English and localize the editor, Outline, CSS Themes, and style-preview document titles through reviewed catalogs.
...
- Document Vault name validation and transaction failures now use an explicit US English allowlist. Users receive reviewed, actionable reasons; unknown provider errors fall back safely without exposing paths or raw system messages.
...
- **Multiple cursors now actually work.** The editor never enabled CodeMirror's
  multiple-selection support, so the multi-cursor behavior `Ctrl+B`/`Ctrl+I`
...
- **The UI now uses the reviewed US English catalog.** Every menu, button,
  tooltip, and message follows the same language policy regardless of document
  content, and nothing users write is translated or transmitted.
- The CSS-theme preview samples use US English so typography remains consistent
...
- **AWS architecture shapes are included**, so a diagram drawn with them shows
  the intended symbols rather than a grid of plain boxes. Shape colors come
...
- **Table cell text can be selected and copied.** Dragging across cells
  highlights their text as ordinary text; previously the press was canceled
