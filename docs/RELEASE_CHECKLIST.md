...
| Production package | `local-markdown-vault-0.2.0.vsix`: 57 files, 3.48 MB (3,651,396 compressed bytes; 14,325,961 uncompressed bytes); archive verifier passed, including the packaged Obsidian compatibility contract |
| VSIX SHA-256 | `77d36f5ac5d91c66f984e52bb78682e063420a9d37a29eefa815b1cc5161b857` |
...
| Dependency audit and SBOM | 0 vulnerabilities; reproducible CycloneDX SBOM validated |
...
remains unsigned while the commit, cross-platform, remaining performance,
accessibility, and independent-review gates below are incomplete.
...
- [ ] Run all unit and performance tests with `npm test`.
...
- [ ] Run the trusted and genuinely untrusted extension-host suites with `npm run test:integration` and `npm run test:integration:restricted`.
- [ ] Run all browser and malicious-content tests with `npm run test:e2e`.
- [ ] Create the production package with `npm run package`, then run `npm run test:vsix`; the archive verifier and clean-profile trusted/Restricted Mode packaged-extension suites must pass.
- [ ] Run `npm run security:audit`; no critical or high finding is accepted.
- [ ] Generate the validated CycloneDX SBOM with `npm run security:sbom` and retain `local-markdown-vault.cdx.json` with the release evidence. The file is excluded from Git and the VSIX because the CI and release workflows retain it as a separate artifact.
...
- [ ] Run the **Cross-platform release validation** workflow for the release commit and retain its three packaged-VSIX artifacts plus the separate release SBOM artifact. A green hosted run supplements but does not replace the manual filesystem and trash checks.
...
- [ ] Install the newly produced VSIX into a clean profile with no development extension loaded.
